...
async function retrieveReadyEmbeddings() {
    ...
    try {
        ...
        for (const row of rows) {
            try {
                const embedding = typeof row.embedding === 'string' ? JSON.parse(row.embedding) : row.embedding;// db stores arrays of floats as JSON strings; this line converts it back to js array so now the embedding = [0.1, 0.2, ...]
                ...
                console.warn(`skipping question  ${row.question_id}: failed to parse embedding JSON`, parseError);
...
export async function findSimilarQuestionsByText({ sourceText, threshold, k }) {
    ...
    const normalizedThreshold = threshold >= 0 && threshold <= 1 ? threshold : RECOMMEND_THRESHOLD;

    let embeddingResult;// its the input question's embedding vector value
    try {
        embeddingResult = await generatingQuestionEmbedding(sourceText, { taskType: "RETRIEVAL_QUERY" });//the returned value is object so embeddingResult= {embedding: [...]}
        ...
        console.error('error:', error);
        console.error('==============');
        throw new ServiceUnavailableError('failed to generate embedding for search query. please try again later.');
    };
    ...
    try {
        storedEmbeddings = await retrieveReadyEmbeddings();/*we are retriving all embeddings of questions which have status = "ready"(means embedding was generated successfully ) to compare it with the current input question's embedding to find similar questions so storedEmbedding = [
               questionId: 1, embedding: [0.1, 0.2, ...]}, 
               questionId: 2, embedding: [0.1, 0.2, ...]}, ...
            ]
        ...
        console.error('== database error during searching for simmilar questions ==');
    ...
    for(const stored of storedEmbeddings){
        try {
            ...
            console.warn(`failed to caculate similarity for questions ${stored.questionId}`,  error.message,
            );
    ...
    const similarQuestions = topResults.filter(result => questionMap[String(result.questionId)]).map(result =>({
...
export const findSimilarQuestionsByQuestionHash=async({questionHash, threshold: searchThreshold, k}) => {
   // ! Normalize k and threshold
    const normalizedK = k > 0 ? Math.min(k, 20) : RECOMMEND_K;
    const normalizedThreshold = searchThreshold >= 0 && searchThreshold <= 1 ? searchThreshold : RECOMMEND_THRESHOLD;
   ...
   try {
    ...
    //console.log("getting question_id for hash", questionHash, ":", rows);
      ...
      console.error(
        '== mysql error finding question by hash =='
      );
      console.error(
   ...
   try {
    ...
    //   console.log("IS ARRAY:", Array.isArray(row));
      ...
      sourceEmbedding = typeof row[0].embedding === 'string'? JSON.parse(row[0].embedding) : row[0].embedding; 
   } catch (error) {
      console.error(
        '== mysql error getting source question embedding =='
      );
      console.error(
   ...
   const otherEmbeddings = allStoredEmbeddings.filter(singleEmbedding => singleEmbedding.questionId !== questionId);
   ...
   try {
      rows = await safeExecute(sql, [questionIds]);/**it may return like this:-  rows = [
                    {
                        questionId: 2,
                        questionHash: "aaa...",
      ...
      console.error(
            '== mysql error fetching top similar questions =='
        );
        console.error(
   ...
   const similarQuestions = topResults
    .filter(result =>
        questionMap[String(result.questionId)]//Does questionMap have questionId 2? YES → keep it so we are checking if db actually returned detailes for each questionId of topResults ena this filter keeps only the actual rresults that successfully retrived
