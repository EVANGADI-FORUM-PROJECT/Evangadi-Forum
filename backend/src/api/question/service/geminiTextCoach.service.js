...
export const generateQuestionDraftCoachService = async({title, content})=>{
    ...
    try {
        ...
        let tips = Array.isArray(parsed?.tips) ? parsed.tips.filter(t => typeof t === 'string' && t.trim()).map(t=> t.trim()) : [];
        tips = tips.slice(0, 5);
        if(tips.length === 0){
            tips = [
        ...
        throw new ServiceUnavailableError('AI draft sugesstions are temporarily unavailable. please try again later.');
    };
};
/* i got :
{
