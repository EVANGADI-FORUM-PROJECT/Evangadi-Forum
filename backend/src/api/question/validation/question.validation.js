...
export const createQuestionValidation = [
  body("title")
    ...
    .isString()
    .withMessage(
      "Question title must be a string",
    )
    .isLength({ min: 5, max: 255 })
    .withMessage(
      "Question title must be between 5 and 255 characters",
    )
    .trim(),
...
export const searchQuestionsSemanticValidation = [
  query("query")
    ...
    .isLength({ min: 5 })
    .withMessage(
      "Search query must be at least 5 characters",
    )
    .trim(),
  ...
  query("threshold")
    ...
    .withMessage(
      "threshold must be a floating point number between 0 and 1",
    )
...
export const similarQuestionsValidation = [
  ...
  query("k")
    ...
    .isInt({ min: 1, max: 20 })
    .withMessage(
      "k must be an integer between 1 and 20",
    )
    .toInt(),
  query("threshold")
